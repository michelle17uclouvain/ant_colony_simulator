...
class SmartStrategy(AntStrategy):
    ...
    def decide_action(self, perception: AntPerception) -> AntAction:
        ...
        if SmartStrategy.shared_food_hints and random.random() <= 0.15:
            dead = []
            for rel in list(SmartStrategy.shared_food_hints.keys()):
                SmartStrategy.shared_food_hints[rel] -= 1
                if SmartStrategy.shared_food_hints[rel] <= 0:
                    dead.append(rel)
            for rel in dead:
                del SmartStrategy.shared_food_hints[rel]
        ...
        if colony_cell is not None:
            ...
            for (dx, dy), terrain in perception.visible_cells.items():
                if terrain == TerrainType.FOOD:
                    rel = (dx - cx, dy - cy)
                    SmartStrategy.shared_food_hints[rel] = SmartStrategy.HINT_TTL
    ...
    def _decide_movement(self, perception: AntPerception) -> AntAction:
        ...
        colony_cell = self._closest_cell(perception, TerrainType.COLONY)
        if colony_cell is not None and SmartStrategy.shared_food_hints:
            top = sorted(SmartStrategy.shared_food_hints.items(), key=lambda x: x[1], reverse=True)[:3]
            if top:
        ...
        return self._wander(perception, state, crowded=crowded)
...
# -------------------------------------------------------------------------------------------- helpers
    ...
    def _wander(self, perception: AntPerception, state: dict, crowded: bool = False) -> AntAction:
        cur = perception.direction.value
        forward_dir = cur
        left_dir = (cur - 1) % 8
        right_dir = (cur + 1) % 8
        ...
        explore_rate = self.EXPLORE_RATE + (0.06 if crowded else 0.0)
